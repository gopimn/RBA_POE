...
signed long x, y, z, timer, dtimer;    //Contain the values of the sensor at each sample
...
byte remote[] = { 192, 168, 1, 150 };                // ip remota
...
void setup() {
  ...
  ads_write_reg(0x4, 0x00);  //OFFSET
  ads_write_reg(0x5, 0x00);  //OFFSET
  ads_write_reg(0x6, 0x00);  //OFFSET
  ads_get_all_regs();
  ads_print_regarray();
